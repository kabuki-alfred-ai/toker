...
import { DownloadClient } from '@/components/features/downloads/download-client'
...
async function fetchData() {
  ...
  const [userRes, historyRes] = await Promise.all([
    ...
    fetch(`${backendUrl}/api/v1/downloads`, { headers, cache: 'no-store' }),
  ])
...
export default async function DownloadPage() {
  ...
  return (
    ...
    <DownloadClient credits={data.credits} history={data.history as any} />
  )
}
