  ...
  private async transcribeAudio(audioPath: string): Promise<{ text: string; segments: { start: number; end: number; text: string }[] }> {
    ...
    if (deepgramKey) {
      try {
        ...
        this.logger.log(`Deepgram response: duration=${data?.metadata?.duration}, channels=${data?.results?.channels?.length}`)
        const text = data?.results?.channels?.[0]?.alternatives?.[0]?.transcript ?? ''
        if (!text) {
          throw new Error(`Deepgram returned empty transcript (confidence: ${data?.results?.channels?.[0]?.alternatives?.[0]?.confidence})`)
        }
        ...
        this.logger.warn(`Deepgram failed: ${err instanceof Error ? err.message : err}. Falling back...`)
      }
    }
    ...
    if (this.groq) {
      try {
        ...
        if (!response.text) throw new Error('Groq returned empty transcript')
        ...
        this.logger.warn(`Groq failed: ${err instanceof Error ? err.message : err}. Falling back...`)
      }
    }
